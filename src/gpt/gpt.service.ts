...
export class GptService {
  ...
  async recommendations(
    ...
    console.log(`${travleRawsJson} 해당 여행지들 중에서 여행지 추천해줘 ${age ? `${age}대들이` : ''} ${people ? `${people}명이서` : ''} ${sdate} 부터 ${edate} 까지 
          ...
          day는 날짜별로 정해줘. 날마다 최소 한곳은 가게해서 무조건 ${sdate} 부터 ${edate} 까지 일정을 다 채울것. 지역이 여러개 있으면 지역마다 들릴것.
           mapLink는 이전 장소랑 다음에 갈 장소인데 출발점 위도,경도,이름, 도착지 위도 경도, 이름 잘 입력해줘 그리고 URL에서 띄어쓰기는+로 채워. 마지막 목적지의 mapLink, distance, estimatedTime는 null로 해.`);
